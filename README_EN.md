...
**1.4.2 fixes connected-WiFi BSSID blocking failing on some Android 14+ builds**: [APK and upgrade notes](../../releases/tag/v1.4.2) · [Changelog](CHANGELOG.md). On builds where diagnostics showed `conn=impl-missing` (the connectivity component is jarjar-renamed), the connected AP's BSSID still leaked and let AMap/Tencent pull the fix back toward the real position; the `ConnectivityService` instance is now taken from its initializer rather than guessed by class name (`conn=ok`). Reboot once after upgrading.

<sub>Earlier: 1.4.1 fixed the spoofed location reverting after a few seconds and scoped-app crashes ([v1.4.1](../../releases/tag/v1.4.1)) · [Changelog](CHANGELOG.md). Streaming location clients (AMap, WeChat, and clone/parallel-space apps) briefly showed the virtual location and then reverted — now every provider report is rewritten (and its mock flag stripped) at the single `onReportLocation` choke point instead of the per-ROM registration classes. **Target apps do not need to be in the module scope and must not be set as exempt** — just tick system framework / phone / bluetooth. Reboot once after upgrading. · [Changelog](CHANGELOG.md). `requestCellInfoUpdate` (Android 10+), the cell identity inside `ServiceState`, and the connected WiFi BSSID that Android 12+ hands out through `NetworkCapabilities` were not blocked before, so an app's own network-positioning SDK could still see the real surroundings — the main reason a few apps / WeChat mini-programs kept showing the original position although the system location was already rewritten. Route planning now lists AMap's alternative routes and accepts via points, and the route polyline is finally visible on the map. **Not retested on hardware**; fully reboot once after upgrading.

1.4 extended WiFi/cell blocking to the newer APIs and added route alternatives + via points; 1.3.6 added "direct delivery"; 1.3.5 fixed `OP_MOCK_LOCATION` being denied on ColorOS/OxygenOS (test providers failing to register); 1.3.4 fixed repeated real-location stalls and the Android 15/16 WiFi-block leak. See the [Changelog](CHANGELOG.md).</sub>
