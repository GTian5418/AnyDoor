...
if (!window.Native) {
  ...
  let running = false, curLat = 39.908722, curLng = 116.397499, route = null, routeStart = 0;
  ...
  window.__mockNative = window.Native = {
    getState: () => JSON.stringify({ config: store.config, app: store.app, started: running, version: '1.0.0-preview',
      service: running ? { running: true, lat: +store.config.lat, lng: +store.config.lng, curLat, curLng, speed: route ? route.speed : 0, bearing: 0, routeActive: !!route, routeDone: route ? Math.min(route.total, (Date.now() - routeStart) / 1000 * route.speed) : 0, routeTotal: route ? route.total : 0, joystick: false, providers: true, mockError: '' } : { running: false } }),
    checkEnv: () => JSON.stringify({ moduleActive: true, systemHook: true, prefsWorldReadable: true, root: true, mockAllowed: true, overlay: false, vectorCli: true, moduleEnabled: true, scope: 'android/0 com.android.phone/0', sdk: 28, device: 'Preview Device / Android 9', mockError: '' }),
    rootSetup: () => '✓ 已授予模拟位置权限\n✓ 已授予悬浮窗权限\nVector: ok\n✓ 已启用模块并设置作用域',
    ...
    startRoute: (j) => { try { const o = JSON.parse(j); let t = 0; for (let i = 1; i < o.points.length; i++) { const a = o.points[i - 1], b = o.points[i]; t += Math.hypot((b.lat - a.lat) * 111320, (b.lng - a.lng) * 88000); } route = { points: o.points, speed: o.speed, total: t }; routeStart = Date.now(); running = true; curLat = o.points[0].lat; curLng = o.points[0].lng; return true; } catch (e) { return false; } },
