...
## 1.4.4 — 2026-09-20（使用姿势卡补充 QQ；措辞更准确）

- 「关于与用法」的「总跳回真实位置」说明卡扩充为 **微信 / QQ / 打卡类**：QQ 与微信同走腾讯定位 SDK、得力走高德 SDK，冷启动跳真实、小窗/保持后台就稳——是这类定位 SDK 的通用行为（冷启动先按公网 IP 发网络定位），不是针对某个 app。
- 措辞更正：真正把定位拉回真实的是**应用自带 SDK 的 IP 网络定位**（在应用进程内、用真实公网 IP），不是 Google Play 服务；因此"冷启动全屏也稳"只能靠目标地区的加速/VPN 改公网 IP。
- 纯文案更新，无逻辑改动。

## 1.4.3 — 2026-09-20（新增赞赏支持入口；补充「打卡类应用总跳回真实位置」的使用姿势）

- 「关于与用法」页与侧栏底部新增「赞赏支持 ☕」入口，点开显示作者微信收款码，可「保存到相册」（存到 `Pictures/AnyDoor` 后触发相册扫描），再用微信「扫一扫 → 相册」赞赏。完全自愿，不打赏也不影响任何功能。
- 「关于与用法」新增一张说明卡：个别应用（得力e+、微信等）在**全屏冷启动**时会先用自带定位按公网 IP 抓一个真实位置——这一步在应用自己进程里、任意门够不着。给出可操作的姿势：先设好位置再开应用、用**小窗/分屏**打开、**别划掉后台**（保持热启动）、冻结 Google Play 服务定位；跨省才跳则是服务器按公网 IP 校正，只能靠目标地区 VPN，任意门改不了 IP。
- 纯界面新增，未改动定位/屏蔽逻辑。
